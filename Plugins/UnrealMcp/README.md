...
`unreal.knowledge_search`, `unreal.tool_recommend`, `unreal.tool_gap_analyze`, `unreal.workflow_recommend`, and `unreal.knowledge_eval_run` are the local RAG-facing planning tools. Chat builds a compact RAG/tool-planning capsule before AI turns and uses the index's machine-readable `missing|empty|stale|ready|corrupt` status to decide whether to refresh and retry. KnowledgeIndex v2 uses atomic same-directory replacement, preserves last-known-good state when a real refresh would produce zero cards, and records hashes, freshness/build metadata, plus source-kind/engine counts. ActivityLog indexing is off by default, promoted local markdown has a separate toggle, search is boundary/version/diversity aware, and eval cases can assert result rank. The 5.7 and 5.8 official-doc manifests and local caches remain separate. The index is written under `Saved/UnrealMcp/KnowledgeIndex`, while fetched official documentation caches stay under `Saved/UnrealMcp/KnowledgeSources`.
